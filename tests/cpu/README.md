...
| 6.6a | Adds **`mcountinhibit`** (CSR 0x320) — plain RW, no new generator surface. Bit 1 (`tm`) and bits 13..31 are WPRI; the rest (`cy`, `ir`, `hpm[12:3]`) are sw-writable storage in our CsrFile. Adapter keeps a `mcountinhibit` wire for the downstream HPM counter-gating logic, now sourced from `hwif_out.mcountinhibit_rdata_flat`. A new `mcountinhibit_csrfile.S` writes `0xFFFFFFFF`, confirms only the 11 implemented bits stick, then round-trips csrrc / csrrs to verify WPRI bit 1 stays zero through a set attempt. | `test_cpu_programs.py[mcountinhibit_csrfile]` |
| 6.6b | Adds **`mcycle` + `mcycleh`** (CSRs 0xB00/0xB80) via a new generator feature: `riscv_hw_increment_when = "cycle_en"` on the low-half field makes the generator emit a `cycle_en: in Bool` port on the CsrFile plus an `if cycle_en -> state <= state +% 1` seq-block line. The high half uses `riscv_hw_increment_high_of = "mcycle"` so it increments on low-half rollover (`cycle_en && low == 0xFFFF_FFFF`). SW writes take priority via last-write-wins. The adapter drops Ibex's `mcycle_counter_i` (ibex_counter instance), wires `cycle_en = mhpmcounter_incr[0] & ~mcountinhibit[0]`, and keeps a shim `mcycle_counter_i` scope exposing a 64-bit `counter_val_o = {mcycleh, mcycle}` so `ibex_core.sv`'s RVFI hierarchical trace still resolves. `mcycle_csrfile.S` verifies: counter running advances, `mcountinhibit.cy` freezes, unfreezing resumes, SW writes land on both halves independently. | `test_cpu_programs.py[mcycle_csrfile]` |
| 6.7 | Adds the **M-mode debug CSRs** — `dcsr` (0x7B0), `dpc` (0x7B1), `dscratch0` (0x7B2), `dscratch1` (0x7B3). Upstream's four `u_{dcsr, depc, dscratch0, dscratch1}_csr` instances go away; RDL + generator handle all the bit-forcing the adapter used to do by hand: `dcsr.xdebugver` is a `sw=r;hw=r;reset=0x4` constant, `cause`/`nmip` are `sw=r;hw=w` (tied zero since DbgTriggerEn=0 never triggers HW write), `stepie`/`stopcount`/`stoptime`/`mprven`/zero-fields are WPRI, `prv` is WARL `{0, 3}` (legalizes invalid writes to U). Adapter rewires `csr_depc_o` from `hwif_out.dpc_dpc`, `debug_{step, ebreakm, ebreaku}_o` from matching `hwif_out.dcsr_*` members, and the `dret` path's `priv_lvl_d = priv_lvl_e'(dcsr_prv)` restore. Debug CSRs are inaccessible from M-mode per spec (Ibex raises illegal-CSR when `debug_mode_i=0`), so there's no SW exercise; correctness is attested by every 6.6a program still passing + a run-to-completion sanity test confirming the four register nodes resolve at the expected hierarchical path. | `test_cpu_programs.py[debug_csrs_csrfile]` |
| 6.8 | Adds the **HPM counter block** — `mhpmcounter3..12` (CSRs 0xB03..0xB0C), `mhpmcounter3h..12h` (0xB83..0xB8C), and `mhpmevent3..12` (0x323..0x32C) — 30 registers matching our `MHPMCounterNum=10` Ibex config. Counter pairs reuse the Phase-6.6b `riscv_hw_increment_when` / `riscv_hw_increment_high_of` UDPs; each counter N has its own `hpmN_inc_en` port on the CsrFile, wired in the adapter to `mhpmcounter_incr[N] & ~mcountinhibit[N]` (identical formula to upstream). Event selectors are `sw=r;hw=w;reset=1<<(N-3)` with the adapter tying hwif_in to the same constant every cycle — matches Ibex's hardwired encoding without needing a sw=r;hw=r path (which arch-com elides, confusing Verilator's lint). Upstream's per-counter `ibex_counter` instances (10 of them, plus mcycle's from 6.6b) are gone; the `mhpmcounter[Cnt]` wire referenced by `ibex_core.sv` RVFI tracing stays driven from `{hwif_out.mhpmcounterNh, hwif_out.mhpmcounterN}` so hierarchical refs resolve unchanged. `hpm_csrs_csrfile.S` verifies RO mhpmevent behavior + mhpmcounter round-trips under `mcountinhibit.hpm` freeze. | `test_cpu_programs.py[hpm_csrs_csrfile]` |
