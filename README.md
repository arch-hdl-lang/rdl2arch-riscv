...
- 🚧 Phase 6 — CPU integration (lowRISC Ibex + our CLINT/PLIC as a SoC).
  ...
  - ✅ Phase 6.8 — **HPM counters** (`mhpmcounter3..12`, `mhpmcounter3h..12h`, `mhpmevent3..12`) — 30 registers matching our `MHPMCounterNum=10` Ibex config. Counter pairs reuse the Phase-6.6b `riscv_hw_increment_when` / `riscv_hw_increment_high_of` UDPs; each counter N gets its own enable port `hpmN_inc_en` wired in the adapter to `mhpmcounter_incr[N] & ~mcountinhibit[N]`. Event selectors are `sw=r;hw=w` with reset `1 << (N - 3)`, and the adapter ties their `hwif_in` to the same constant every cycle — matches Ibex's hardwired encoding. Ibex's `ibex_counter` instances for indices 3..12 go away (10 gone, plus mcycle's from 6.6b = 11 total); the `mhpmcounter[Cnt]` wire referenced by RVFI tracing stays driven from `{hwif_out.mhpmcounterNh, hwif_out.mhpmcounterN}` so RVFI hierarchical refs resolve unchanged. `hpm_csrs_csrfile.S` verifies mhpmevent RO behaviour + mhpmcounter round-trips under mcountinhibit freeze.
