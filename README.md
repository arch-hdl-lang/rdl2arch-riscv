...
- 🚧 Phase 6 — CPU integration (lowRISC Ibex + our CLINT/PLIC as a SoC).
  ...
  - 🚧 Phase 6.5 — Swap Ibex's M-trap CSRs onto our generated `CsrFile`. Ibex's `ibex_cs_registers.sv` is forked in-tree (`tests/cpu/soc/ibex_cs_registers_hybrid.sv`) and upstream's copy is filtered out of the fusesoc filelist. The fork keeps the upstream module name so `ibex_core.sv` binds it unchanged.
    - ✅ Phase 6.5a — **`mscratch`** migrated. Our generated `MTrapMscratchCsrFile` is instantiated inside the forked cs_registers and owns CSR 0x340's storage; upstream's `u_mscratch_csr` is removed entirely. A `mscratch_csrfile.S` program round-trips two patterns through `csrrw mscratch, …`; cocotb asserts the readbacks match AND peeks the generated CsrFile's `mscratch_r.value` hierarchically to confirm the data landed in our state, not a shadow. Phases 6.5b–e will migrate `mtvec` / `mepc`+`mcause`+`mtval` / `mstatus` / `mie`+`mip` following the same pattern.
